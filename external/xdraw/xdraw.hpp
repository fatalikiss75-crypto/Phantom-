...
{
	dark_space,    // Default dark gaming theme
	neon_cyber,    // Neon purple/blue theme
	deep_ocean,    // Dark blue theme
	midnight_black // Pure black theme
};

namespace tokens {

	// Base colors (Phantom cc Theme Palette)
	inline xdraw::color col_accent{ 235, 238, 245, 255 };         // Phantom White accent
	inline xdraw::color col_accent_active{ 255, 255, 255, 255 };  // Active white hover
	inline xdraw::color col_accent_picker{ 235, 238, 245, 255 };  // Accent picker
	inline xdraw::color col_yellow_accent{ 235, 238, 245, 255 };  // Neutral accent
	inline xdraw::color col_dark{ 8, 8, 10, 245 };                // Near-black background
	inline xdraw::color col_text{ 245, 246, 250, 255 };           // Near-white text
	inline xdraw::color col_text_dim{ 150, 152, 160, 220 };       // Muted gray text
	inline xdraw::color col_card{ 18, 18, 22, 235 };             // Card background
	inline xdraw::color col_elevated{ 28, 28, 34, 240 };         // Elevated panel background
	inline xdraw::color col_border{ 255, 255, 255, 38 };         // Subtle white border

	// UI elements
	inline xdraw::color col_sidebar{ 10, 10, 12, 245 };          // Sidebar background
	inline xdraw::color col_header{ 12, 12, 16, 235 };
	inline xdraw::color col_dropdown{ 18, 18, 22, 250 };
	inline xdraw::color col_glow{ 255, 255, 255, 90 };

	// Layout
	constexpr auto sidebar_w{ 185.0f };
	constexpr auto tab_icon_size{ 36.0f };
	constexpr auto subtab_bar_h{ 52.0f };
	constexpr auto gap{ 8.0f };
	constexpr auto card_rounding{ 10.0f };
	constexpr auto btn_rounding{ 6.0f };

	// Interface settings (will be exposed in Settings → Interface)
	inline bool menu_glow_enabled{ true };
	inline bool watermark_enabled{ true };
	inline float animation_speed{ 14.0f };
