...
namespace xui {
	...
	bool checkbox( std::string_view label, setting& s )
	{
		...
		const auto highlight_anim = anim::lerp( id + 97, is_highlighted ? 1.0f : 0.0f, 18.0f );
				...
				// Checkbox glow (behind)
		{
			const auto glow_a = std::min( 160.0f, ease_t * 75.0f + hover_anim * 50.0f );
			if ( glow_a > 2.0f )
			{
				auto glow = tokens::col_glow;
				glow.a = static_cast< std::uint8_t >( glow_a );
				dl.rect_filled( abs.x - 2.0f, abs.y - 2.0f, abs.w + 4.0f, abs.h + 4.0f, glow, xdraw::corner_radius{ st.checkbox_rounding + 2.0f } );
			}
		}
	...
	namespace {
		...
		inline bool slider( std::uintptr_t id, std::string_view label, T& v, T v_min, T v_max, std::string_view fmt )
		{
			...
			// Slider handle + glow
			{
				const auto thumb_cx = track_rect.x + std::clamp( norm_pos, 0.0f, 1.0f ) * track_rect.w;
				const auto thumb_cy = track_rect.y + track_rect.h * 0.5f;
				const auto is_active = ( c.active_slider == id );
				const auto glow_a = 45.0f + hover_anim_val * 55.0f + ( is_active ? 50.0f : 0.0f );
				auto glow = tokens::col_glow;
				glow.a = static_cast< std::uint8_t >( std::min( 160.0f, glow_a ) );
				dl.circle_filled( thumb_cx, thumb_cy, thumb_r + 5.0f, glow );
				dl.circle_filled( thumb_cx, thumb_cy, thumb_r + 2.0f, glow.alpha( static_cast< std::uint8_t >( glow.a * 0.6f ) ) );
				dl.circle_filled( thumb_cx, thumb_cy, thumb_r, s.slider_fill );
			}
	...
	bool combo( std::string_view label, int& current, const char* const items[ ], int count, float width )
	{
		...
		const auto arrow_col = lerp( s.combo_arrow, s.accent, hover_anim );
