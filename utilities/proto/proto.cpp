...
namespace proto {
	...
	{
		// See mutable_viewangles( ) below: never set the has-bit over a null field.
	...
	{
		// Real protobuf allocates the submessage here; this shim only hands back
		// the field that already exists. Setting the has-bit unconditionally told
		// the game "viewangles is present" while m_viewangles stayed null, so the
		// read that followed once the usercmd was consumed faulted at a small
		// offset off null -- after create_move had already returned cleanly.
		// Callers still null-check the result; this stops a null field from
		// poisoning the message on the way out.
	...
	{
		// See base_usercmd_pb::mutable_viewangles( ): never set the has-bit over a
		// null field.
		if ( !this->m_base )
		{
			return nullptr;
